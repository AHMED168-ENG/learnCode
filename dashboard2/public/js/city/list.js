...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard2/city/new" class="btn btn-info">Add new</a>`)
        }
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>
                    <a href="/dashboard2/city/edit/${elem.city_id}"><i class="fas fa-edit text-primary"></i></a></td>
                </tr>`)
            })
        } else {
            res.data.forEach((elem) => {
