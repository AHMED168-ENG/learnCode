...
function getList() {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard2/tree/header/new" class="btn btn-info">Add new</a>`);
        }
        $("#tr-th-row").empty()
        if (res.canEdit) {
            res.data.forEach((elem) => {
                $("#tr-th-row").append(
                    `<tr>
                        ...
                        <td>${elem.en_name}</td>
                        <td>${elem.ar_name}</td>
                        <td><a href="/dashboard2/tree/header/edit/${elem.id}"><i class="fas fa-edit text-primary"></i></a></td>
                    </tr>`
                )
            })
        } else {
