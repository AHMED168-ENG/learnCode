        ...
        trees
          ...
          .then(async (count) => {
            ...
            if (!isHighestAdmin) {
              ...
              canEdit = userPermissions.filter((per) => per["tbl_page"]["type"] === "Edit" && per["tbl_page"]["tbl_module"]["name"] === "Trees List");
              canAdd = userPermissions.filter((per) => per["tbl_page"]["type"] === "Add" && per["tbl_page"]["tbl_module"]["name"] === "Trees List");
            }
            const dataInti = {
              total: count,
              limit: limit,
