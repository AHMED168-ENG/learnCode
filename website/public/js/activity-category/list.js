...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/website/activity-category/new" class="btn btn-info">Add new</a>`)
        }
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>${elem.ar_name}</td>
                <td>
                    <a class="m-1" data-tooltip="View Activity Category" href="/website/activity-category/view/${elem.id}"><i class="fas fa-eye"></i></a>
                    <a class="m-1" href="/website/activity-category/edit/${elem.id}"><i class="fas fa-edit text-primary"></i></a></td>
                </tr>`)
            })
        } else {
            res.data.forEach((elem) => {
                ...
                <td>${elem.ar_name}</td>
                <td>
                    <a class="m-1" data-tooltip="View Activity Category" href="/website/activity-category/view/${elem.id}"><i class="fas fa-eye"></i></a></td>
                </tr>`)
            })
        }
