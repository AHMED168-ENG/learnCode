...
export class CityController {
  listPage(req: Request, res: Response, next: NextFunction) {
    res.render("dashboard/views/city/list.ejs", {
      title: "City",
    })
  }
        ...
        city
          ...
          .then(async (count) => {
            ...
            if (!isHighestAdmin) {
              ...
              canEdit = !!userPermissions.filter((per) => per["tbl_page"]["type"] === "Edit" && per["tbl_page"]["tbl_module"]["name"] === "Cities List").length;
              canAdd = !!userPermissions.filter((per) => per["tbl_page"]["type"] === "Add" && per["tbl_page"]["tbl_module"]["name"] === "Cities List").length;
            }
            const dataInti = {
              total: count,
              limit: limit,
  ...
  async newPage(req: Request, res: Response, next: NextFunction) {
    ...
    res.render("dashboard/views/city/new.ejs", {
      title: "City new",
      countries,
    })
  }
  ...
  async editPage(req: Request, res: Response, next: NextFunction) {
    ...
    city.findOne({where: {city_id: id}, attributes: ["country_id", "ar_name", "en_name", "code"], raw: true}).then((data) => {
      res.render("dashboard/views/city/edit.ejs", {
        title: "Edit City",
        data: data,
        countries,
      })
