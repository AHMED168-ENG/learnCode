...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard/company-offer/${res.company_id}/new" class="btn btn-info">Add new</a>`)
        }
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>${elem.en_name}</td>
                <td>${elem.ar_name}</td>
                <td>${elem['tbl_rental_company']['name']}</td>
                <td>${elem.price}</td>
                <td>
                    <a class="m-1" data-tooltip="View Company Offer" href="/dashboard/company-offer/${elem.rental_company_id}/view/${elem.id}"><i class="fas fa-eye"></i></a>
                    <a class="m-1" href="/dashboard/company-offer/${elem.rental_company_id}/edit/${elem.id}"><i class="fas fa-edit text-primary"></i></a>
                </td>
                </tr>`)
            })
        } else {
