...
export class AdsController {
  ...
  public listPage(req: Request, res: Response, next: NextFunction) {
    return res.render("website/views/ads/list.ejs", { title: "Advertisements" });
  }
  public async list(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const dataInti = { total: countAds, limit, page: Number(req.query.page), pages: Math.ceil(countAds / limit) + 1, data, canEdit: permissions.canEdit, module_id };
  ...
  public async viewPage(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const module_id = await new ModulesController().getModuleIdByName("Ads Management");
      return res.render("website/views/ads/view.ejs", { title: "View Ad Details", data, module_id });
    } catch (error) {
