...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard2/promo/new" class="btn btn-info">Add new</a>`);
        }
        $("#tr-th-row").empty()
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>${elem.promo_type == "all_users" ? "All users" : `<a  class="text-dark" href='/dashboard2/user/edit/${elem.user_id}'>${elem.web_apps_user.fullName}</a>`}</td>
                ...
                <td>
                    <a data-tooltip="Edit promo" class="btn btn-primary" href="/dashboard2/promo/edit/${elem.promo_id}"><i class="fas fa-edit"></i></a>
