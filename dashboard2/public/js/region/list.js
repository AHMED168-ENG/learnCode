...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard2/region/new" class="btn btn-info">Add new</a>`);
        }
        $("#tr-th-row").empty()
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>
                    <a href="/dashboard2/region/edit/${elem.region_id}"><i class="fas fa-edit text-primary"></i></a></td>
                </tr>`)
            })
        } else {
            res.data.forEach((elem) => {
