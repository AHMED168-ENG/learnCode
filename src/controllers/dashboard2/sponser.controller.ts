...
export class SponserController {
  listPage(req: Request, res: Response, next: NextFunction) {
    res.render("dashboard/views/sponser/list.ejs", {
      title: "Sponsers",
    })
  }
      ...
      .then((data) => {
        ...
        .then(async (count) => {
            ...
            if (!isHighestAdmin) {
              ...
              canEdit = !!userPermissions.filter((per) => per["tbl_page"]["type"] === "Edit" && per["tbl_page"]["tbl_module"]["name"] === "Sponsors").length;
              canAdd = !!userPermissions.filter((per) => per["tbl_page"]["type"] === "Add" && per["tbl_page"]["tbl_module"]["name"] === "Sponsors").length;
            }
            const dataInti = {
              total: count,
              limit: limit,
  ...
  newPage(req: Request, res: Response, next: NextFunction) {
    res.render("dashboard/views/sponser/new.ejs", {
      title: "Sponsers new",
    })
  }
  ...
  async editPage(req: Request, res: Response, next: NextFunction) {
    ...
    sponser.findOne({where: {sponser_id: id}, attributes: {exclude: ["password", "updatedAt", "createdAt"]}, raw: true}).then((data) => {
      res.render("dashboard/views/sponser/edit.ejs", {
        title: "Edit Sponsor",
        data: data,
      })
    })
