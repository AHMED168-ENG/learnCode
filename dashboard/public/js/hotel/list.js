...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        // if (res.canAdd) {
        //     $('#addNewBtn').html(`<a type="button" href="/dashboard/hotel/new" class="btn btn-info">Add new</a>`)
        // }
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>
                    <a class="m-1" data-tooltip="View Hotel" href="/dashboard/hotel/view/${elem.id}"><i class="fas fa-eye"></i></a></td>
                </tr>`)
            })
        } else {
            res.data.forEach((elem) => {
