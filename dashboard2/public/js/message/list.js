...
function getList(page) {
    ...
    $.ajax(settings).done(function (res) {
        ...
        res.data.forEach((elem) => {
            ...
            <td><a href="/dashboard2/message/view/${elem.message_id}" data-tooltip="View message" class="btn btn-info"><i class="fas fa-eye"></i></a>
            </td>
            </tr>`)
...
function changeStatus(id, status) {
    const settings = {
        async: true,
        crossDomain: true,
        url: `/dashboard2/message/status/${id}?status=${status}`,
        method: "PUT",
    }
