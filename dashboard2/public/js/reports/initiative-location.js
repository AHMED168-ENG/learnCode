...
function getList(type = "all", from = null, to = null) {  // show spinner
    ...
    const settings = {
        async: true,
        crossDomain: true,
        url: `/dashboard2/report/${type}/locationlistreport?from=${from}&to=${to}`,
        method: "Get",
    }
    $.ajax(settings).done(function (res) {
        ...
        changeChart(res.initiativesChart, selectedMonths);
        spinnerNotfound(2)
        $("#tr-th-row").empty()
...
function changeChart(inYear, selectedMonths) {
    ...
    window.bar = new Chart($("#initLocationDetails").get(0).getContext("2d"), {
        type: "bar",
        data: chartSet,
