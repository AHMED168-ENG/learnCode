...
function getList() {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        spinnerNotfound(2)
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard/travel/new" class="btn btn-info">Add new</a>`)
        }
        $("#accordion").empty();
        res.data.forEach((category) => {
            ...
            if (res.canEdit) {
                $('#editBtn').append(`<a type="button" href="/dashboard/travel/edit/${category.id}" class="btn btn-info">Edit ${category.en_name}</a>`);
            }
        })
