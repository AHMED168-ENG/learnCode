...
function getList(page) {
    ...
    $.ajax(settings).done(function (res) {
        ...
        res.data.forEach((elem) => {
            ...
            <td><a href="/dashboard2/user/edit/${elem.user_id}" class="btn btn-link text-dark">${elem.user_name}</a></td>
            ...
            if (screenType == "new") {
                if (res.canViewNewOrders) {
                    $(`#${elem.order_id} td:last`).append(`<a href="/dashboard2/order/view/${elem.order_id}" data-tooltip="View order" class="btn btn-info"><i class="fas fa-eye"></i></a>`)
                }
                ...
                $(`#${elem.order_id} td:last`).append(`
                 <a href="/dashboard2/order/view/${elem.order_id}" data-tooltip="View order" class="btn btn-info"><i class="fas fa-eye"></i></a>`)
...
function changeStatus(id, status) {
    const settings = {
        async: true,
        crossDomain: true,
        url: `/dashboard2/order/status/${id}?status=${status}`,
        method: "PUT",
    }
