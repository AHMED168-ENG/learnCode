    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>${elem.tbl_trip.ar_name} - ${elem.tbl_trip.en_name}</td>
                <td>
                    <a class="m-1" data-tooltip="View Tour Guide" href="/dashboard/trip/view/${elem.trip_id}"><i class="fas fa-eye"></i></a>
            ...
            res.data.forEach((elem) => {
                ...
                <th scope="row">${elem.id}</th>
                <td>${elem.tbl_trip.ar_name} - ${elem.tbl_trip.en_name}</td>
                <td><a class="m-1" data-tooltip="View Tour Guide" href="/dashboard/trip/view/${elem.trip_id}"><i class="fas fa-eye"></i></a></td>
                </tr>`)
            })
        }
