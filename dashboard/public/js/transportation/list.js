    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td>${elem['name']}</td>
                <td>${elem['type']}</td>
                <td>${elem['tbl_driver']['name']}</td>
                <td>${elem['tbl_rental_company']['name']}</td>
                <td>
                    <a class="m-1" data-tooltip="View Transportation" href="/dashboard/transportation/view/${elem.id}"><i class="fas fa-eye"></i></a>
            ...
            res.data.forEach((elem) => {
                ...
                <td>${elem['tbl_driver']['name']}</td>
                <td>${elem['tbl_rental_company']['name']}</td>
                <td><a class="m-1" data-tooltip="View Transportation" href="/dashboard/transportation/view/${elem.id}"><i class="fas fa-eye"></i></a></td>
                </tr>`)
            })
        }
