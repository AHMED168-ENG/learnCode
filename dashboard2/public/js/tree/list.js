...
function getList(page) {
    ...
    $.ajax(settings).done(function (res, textStatus) {
        ...
        if (res.canAdd) {
            $('#addNewBtn').html(`<a type="button" href="/dashboard2/tree/new" class="btn btn-info">Add new</a>`);
        }
        $("#tr-th-row").empty()
        if (res.canEdit) {
            res.data.forEach((elem) => {
                ...
                <td><a class="pr-2" href="/dashboard2/tree/details/${elem.tree_id}"><i class="fas fa-eye text-success"></i></a><a class="pr-2" href="/dashboard2/tree/edit/${elem.tree_id}"><i class="fas fa-edit text-primary"></i></a></td>
                </tr>`)
            })
        } else {
            res.data.forEach((elem) => {
